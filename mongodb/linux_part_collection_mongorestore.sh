...
for item in "${collection_array[@]}"; do
    ...
    for value in $(echo "$group_output" | grep -o '"_id" : "[^"]*' | grep -o '[^"]*$'); do
        ...
        mongorestore --gzip --host $mongo_host --port $mongo_port --username $mongo_username --password $mongo_password -d $mongo_db --dir "$backup_folder" --nsFrom="$mongo_db.$item" --nsTo="$mongo_db.$value" --drop
