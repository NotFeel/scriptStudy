...
%file_path%\mongodb\bin\mongodump.exe --gzip -h %mongo_host%:%mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% --db %source_db% --out %dump_path% > null

%file_path%\mongodb\bin\mongorestore.exe --gzip -h %mongo_host%:%mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% --db %target_db% --dir %dump_path%/%source_db% --drop > null
...
for %%A in (%collection_array%) do (
    ...
    for /f "usebackq delims=" %%B in (`%file_path%\mongodb\bin\mongo.exe --host %mongo_host% --port %mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% %target_db% --eval "DBQuery.shellBatchSize=1000;!mongo_group_command!" ^| findstr "_id"`) do (
        ...
        for /f "tokens=2 delims=:\}" %%C in ("!line!") do (
            ...
            %file_path%\mongodb\bin\mongodump.exe --gzip -h %mongo_host%:%mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% --db %target_db% -c %%A --query "{ 'elementName': '!value!' }" --out "!backup_folder!" > null
        )
    )
)
