...
for %%A in (%collection_array%) do (
    ...
    for /f "usebackq delims=" %%B in (`%file_path%\mongodb\bin\mongo.exe --host %mongo_host% --port %mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% %target_db% --eval "!mongo_group_command!" ^| findstr "_id"`) do (
        ...
        for /f "tokens=2 delims=:\}" %%C in ("!line!") do (
            ...
            %file_path%\mongodb\bin\mongo.exe --host %mongo_host% --port %mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% %source_db% --eval "!create_collection_command!"

            set backup_folder=%file_path%\collection\!value!\%target_db%
            %file_path%\mongodb\bin\mongorestore.exe --gzip -h %mongo_host%:%mongo_port% --authenticationDatabase admin -u %mongo_username% -p %mongo_password% -d %target_db% --dir "!backup_folder!" --nsFrom="%target_db%.%%A" --nsTo="%source_db%.!value!" --drop
        )
    )
)
