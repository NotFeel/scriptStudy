...
for value in "${elementName[@]}"; do
    ...
    mongodump --gzip --host $mongo_host --port $mongo_port --username $mongo_username --password $mongo_password --db $mongo_db --collection default --query "{ 'elementName': '$value' }" --out "$backup_folder"
done
